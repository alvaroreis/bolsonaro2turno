async function file(file, docName) {
    try {
        ...
        const fileName = `${codeInput.value.trim()}-${president}`;
...
document.querySelector("a#shareLink").addEventListener("click", async (e) => {
    try {
        ...
        const data = {
            title: "Flyer Eleições 2º Turno",
            text: "Gere um Flyer com o seu nome de apoio ao seu favorito 2º no Turno:",
            url: url,
        };
        await share(data);
    } catch (error) {
...
document.querySelector("a#shareImage").addEventListener("click", async () => {
    try {
        ...
        const data = { title: "Flyer Eleições 2º Turno", files: [image] };
